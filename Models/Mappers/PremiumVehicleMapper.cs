...
{
    public static class PremiumVehicleMapper
    {
        public static PremiumVehicleResponseDto ToResponseDto(this PremiumVehicle entity)
        {
            if (entity == null) return null!;

            return new PremiumVehicleResponseDto
            {
                Id = entity.Id,
                Title = entity.Title,
                ...
                Price = entity.Price != null ? new PremiumVehiclePriceInfoDto
                {
                ...
                KeySpecifications = entity.KeySpecifications != null ? new PremiumVehicleKeySpecificationsDto
                {
                ...
                TopFeatures = entity.TopFeatures?.Select(tf => new PremiumVehicleFeatureItemDto
                {
                    Feature = tf.Feature
                }).ToList() ?? new List<PremiumVehicleFeatureItemDto>(),

                StandOutFeatures = entity.StandOutFeatures?.Select(sf => new PremiumVehicleFeatureItemDto
                {
                    Feature = sf.Feature
                }).ToList() ?? new List<PremiumVehicleFeatureItemDto>(),

                Pros = entity.Pros?.Select(p => new PremiumVehicleProConItemDto
                {
                    Pro = p.Pro,
                    Con = p.Con
                }).ToList() ?? new List<PremiumVehicleProConItemDto>(),

                Cons = entity.Cons?.Select(c => new PremiumVehicleProConItemDto
                {
                    Pro = c.Pro,
                    Con = c.Con
                }).ToList() ?? new List<PremiumVehicleProConItemDto>(),

                Tags = entity.Tags?.Select(t => new PremiumVehicleTagItemDto
                {
                    TagName = t.TagName
                ...
                UserRatings = entity.UserRatings?.Select(ur => new PremiumVehicleUserRatingDto
                {
                    UserId = ur.UserId,
                ...
                }).ToList() ?? new List<PremiumVehicleUserRatingDto>(),
                ...
                Location = entity.Location != null ? new PremiumVehicleLocationInfoDto
                {
                ...
                Condition = entity.Condition != null ? new PremiumVehicleConditionDto
                {
                ...
                ListingDetails = entity.ListingDetails != null ? new PremiumVehicleListingDetailsDto
                {
                    IsAvailable = entity.ListingDetails.IsAvailable,
                    IsPremium = entity.ListingDetails.IsFeatured,
                ...
                Engagement = entity.Engagement != null ? new PremiumVehicleEngagementMetricsDto
                {
                ...
                ShareUrls = entity.ShareUrls != null ? new PremiumVehicleShareUrlsDto
                {
                ...
                TestDrive = entity.TestDrive != null ? new PremiumVehicleTestDriveInfoDto
                {
                ...
                IsPremium = entity.ListingDetails?.IsFeatured ?? false,
        ...
        public static List<PremiumVehicleResponseDto> ToResponseDtoList(this IEnumerable<PremiumVehicle> entities)
        {
            return entities?.Select(e => e.ToResponseDto()).ToList() ?? new List<PremiumVehicleResponseDto>();
        }

        public static PremiumVehicleSummaryDto ToSummaryDto(this PremiumVehicle entity)
        {
            if (entity == null) return null!;

            return new PremiumVehicleSummaryDto
            {
                Id = entity.Id,
                Title = entity.Title,
                BrandName = entity.BrandName,
        ...
        public static List<PremiumVehicleSummaryDto> ToSummaryDtoList(this IEnumerable<PremiumVehicle> entities)
        {
            return entities?.Select(e => e.ToSummaryDto()).ToList() ?? new List<PremiumVehicleSummaryDto>();
        }

        public static PremiumVehicle ToEntity(this PremiumVehicleRequestDto request)
        {
            ...
            return new PremiumVehicle
            {
                ...
                TopFeatures = request.TopFeatures?.Select(tf => new FeatureItem { Feature = tf.Feature }).ToList() ?? new List<FeatureItem>(),
                StandOutFeatures = request.StandOutFeatures?.Select(sf => new FeatureItem { Feature = sf.Feature }).ToList() ?? new List<FeatureItem>(),
                Pros = request.Pros?.Select(p => new ProConItem { Pro = p.Pro, Con = p.Con }).ToList() ?? new List<ProConItem>(),
                Cons = request.Cons?.Select(c => new ProConItem { Pro = c.Pro, Con = c.Con }).ToList() ?? new List<ProConItem>(),
                Tags = request.Tags?.Select(t => new TagItem { TagName = t.TagName }).ToList() ?? new List<TagItem>(),
        ...
        public static void UpdateFromRequest(this PremiumVehicle entity, PremiumVehicleRequestDto request)
        {
            ...
            entity.TopFeatures = request.TopFeatures?.Select(tf => new FeatureItem { Feature = tf.Feature }).ToList() ?? new List<FeatureItem>();
            entity.StandOutFeatures = request.StandOutFeatures?.Select(sf => new FeatureItem { Feature = sf.Feature }).ToList() ?? new List<FeatureItem>();
            entity.Pros = request.Pros?.Select(p => new ProConItem { Pro = p.Pro, Con = p.Con }).ToList() ?? new List<ProConItem>();
            entity.Cons = request.Cons?.Select(c => new ProConItem { Pro = c.Pro, Con = c.Con }).ToList() ?? new List<ProConItem>();
            entity.Tags = request.Tags?.Select(t => new TagItem { TagName = t.TagName }).ToList() ?? new List<TagItem>();
        ...
        public static void UpdatePriority(this PremiumVehicle entity, int priority)
        {
        ...
        public static void Activate(this PremiumVehicle entity, DateTime? endDate = null)
        {
            if (entity == null) return;
        ...
        public static void Deactivate(this PremiumVehicle entity)
        {
            if (entity == null) return;
        ...
        public static void AddRating(this PremiumVehicle entity, PremiumVehicleUserRatingDto rating)
        {
        ...
        public static void IncrementEngagement(this PremiumVehicle entity, string metricType)
        {
            if (entity == null) return;
        ...
        public static bool IsCurrentlyPremium(this PremiumVehicle entity)
        {
        ...
        private static double CalculateAverageRating(List<PremiumVehicleUserRatingDto>? ratings)
        {
            if (ratings == null || !ratings.Any())
                return 0;
